	...
	with open("run_logs.log", "a+") as log:
		...
		try:
			reddit = praw.Reddit(client_id=Secrets.REDDIT_CLIENT_ID,
							client_secret=Secrets.REDDIT_CLIENT_SECRET,
	 					    user_agent=Secrets.REDDIT_USERAGENT,
		...
		for submission in subreddit.new(limit=10):
			...
			if submission.domain == "twitter.com":
				...
				soup_filter = str(soup.find_all('meta', {'property':'og:image'}, limit=4))
				print("soup_filter: " + soup_filter)
				image_url = re.findall('con=\"(.*?)\"', soup_filter)
				if image_url == []:
					continue
